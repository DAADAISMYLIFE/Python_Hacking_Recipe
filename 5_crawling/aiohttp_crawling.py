...
json_path = "./5. crawling/comments.json"

def get_product_urls(urls):
    """모든 상품의 URL 반환

    Args:
        urls (문자열): 크롤링할 웹 페이지의 URL
    """
...
async def async_func(urls):
    ...
    async with aiohttp.ClientSession(connector=conn) as s:
        futures = [asyncio.create_task(show_product_reviews(s, url)) for url in urls]
        results = await asyncio.gather(*futures)
    with open(json_path, "w", encoding="utf-8") as f:
        print(f"JSON file saved as : {json_path}")
        json.dump(results, f, indent=4, ensure_ascii=False)

async def show_product_reviews(s, url):
...
if __name__ == "__main__":
    ...
    # Windows에서 SelectorEventLoop 설정
    if os.name == "nt":
