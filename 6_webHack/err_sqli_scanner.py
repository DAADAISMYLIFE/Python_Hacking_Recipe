...
target_url = f"{base_url}/sqli_1.php"
...
def get_forms(session, url):
    ...
    """
    page_content = session.get(url, verify=False).content
...
def sql_injection(session, form_details, url, value):
    ...
    try:
        if form_details["method"] == "get":
            ...
            response = session.get(joined_url, params=data, verify=False).content.decode()
            ...
            response = session.post(target_url, data=data, verify=False).content.decode()
            is_vulnerable, db_type = check_sql(response)
